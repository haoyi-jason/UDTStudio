...
void BMS_Logger::generateRecord(BCU *bcu)
{
    ...
    ds << QString(",%1").arg(bcu->state()); // state
    ds << QString(",%1").arg(bcu->maxCell()); // max cell
    ds << QString(",%1").arg(bcu->minCell()); // min cell
    ds << QString(",%1").arg(bcu->maxCellPos()); // max pos
    ds << QString(",%1").arg(bcu->minCellPos()); // min pos
    ...
    ds << QString(",%1").arg(bcu->maxTempPos()); // max t pos
    ds << QString(",%1").arg(bcu->minTempPos()); // min t pos
