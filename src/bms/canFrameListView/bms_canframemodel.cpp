...
BMS_CanFrameModel::BMS_CanFrameModel(QObject *parent)
...
BMS_CanFrameModel::~BMS_CanFrameModel()
{
}

void BMS_CanFrameModel::appendCanFrame(const QCanBusFrame &frame)
{
...
CanOpenBus *BMS_CanFrameModel::bus() const
{
    return _bus;
}

void BMS_CanFrameModel::setBus(CanOpenBus *bus)
{
    emit layoutAboutToBeChanged();
    if (_bus != nullptr)
    {
        disconnect(_bus, &CanOpenBus::frameAvailable, this, &BMS_CanFrameModel::updateFrames);
    }
    _bus = bus;
    _frameId = _bus->canFramesLog().count();
    connect(bus, &CanOpenBus::frameAvailable, this, &BMS_CanFrameModel::updateFrames);
    emit layoutChanged();
}

void BMS_CanFrameModel::updateFrames(int id)
{
...
int BMS_CanFrameModel::columnCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent)
    return ColumnCount;
}

QVariant BMS_CanFrameModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
QVariant BMS_CanFrameModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
    {
        return QVariant();
...
QModelIndex BMS_CanFrameModel::index(int row, int column, const QModelIndex &parent) const
{
    Q_UNUSED(parent)
    if (_bus == nullptr)
    {
...
QModelIndex BMS_CanFrameModel::parent(const QModelIndex &child) const
{
    Q_UNUSED(child)
    return QModelIndex();
}

int BMS_CanFrameModel::rowCount(const QModelIndex &parent) const
{
    if (!parent.isValid())
    {
        if (_bus == nullptr)
...
Qt::ItemFlags BMS_CanFrameModel::flags(const QModelIndex &index) const
{
    if (!index.isValid())
    {
