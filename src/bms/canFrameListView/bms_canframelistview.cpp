...
#include "bms_canframelistview.h"
...
BMS_CanFrameListView::BMS_CanFrameListView(QWidget *parent)
    : QTableView(parent)
{
    _canModel = new BMS_CanFrameModel();
    setSelectionBehavior(QAbstractItemView::SelectRows);
    setModel(_canModel);
    setVerticalScrollMode(ScrollPerPixel);

    connect(selectionModel(), &QItemSelectionModel::selectionChanged, this, &BMS_CanFrameListView::updateSelect);
...
#endif
    horizontalHeader()->resizeSection(BMS_CanFrameModel::Time, 10 * w0);
    horizontalHeader()->resizeSection(BMS_CanFrameModel::CanId, 12 * w0);
    horizontalHeader()->resizeSection(BMS_CanFrameModel::Type, 8 * w0);
...
#endif
    horizontalHeader()->resizeSection(BMS_CanFrameModel::DataByte, 9 * w1);
...
void BMS_CanFrameListView::appendCanFrame(const QCanBusFrame &frame)
{
...
void BMS_CanFrameListView::clear()
{
    _canModel->clear();
}

void BMS_CanFrameListView::copy()
{
    QString text;
...
void BMS_CanFrameListView::updateSelect(const QItemSelection &selected, const QItemSelection &deselected)
{
    Q_UNUSED(selected)
    Q_UNUSED(deselected)
...
void BMS_CanFrameListView::createActions()
{
...
#endif
    connect(_clearAction, &QAction::triggered, this, &BMS_CanFrameListView::clear);
    ...
    connect(_copyAction, &QAction::triggered, this, &BMS_CanFrameListView::copy);
    addAction(_copyAction);
}

QAction *BMS_CanFrameListView::copyAction() const
{
    return _copyAction;
}

QAction *BMS_CanFrameListView::clearAction() const
{
    return _clearAction;
}

void BMS_CanFrameListView::contextMenuEvent(QContextMenuEvent *event)
{
    QMenu menu;
...
BMS_CanFrameListView::~BMS_CanFrameListView()
{
    delete _canModel;
}

CanOpenBus *BMS_CanFrameListView::bus() const
{
    return _canModel->bus();
}

void BMS_CanFrameListView::setBus(CanOpenBus *bus)
{
