...
BMS_DataLoggerTreeView::BMS_DataLoggerTreeView(QWidget *parent)
    : QTreeView(parent)
{
    HeaderView *headerView = new HeaderView(Qt::Horizontal, this);
    headerView->addMandatorySection(BMS_DataLoggerModel::NodeName);
    setHeader(headerView);

    _loggerModel = new BMS_DataLoggerModel(this);
...
BMS_DataLoggerTreeView::~BMS_DataLoggerTreeView()
{
}

DLData *BMS_DataLoggerTreeView::currentData() const
{
...
BMS_DataLogger *BMS_DataLoggerTreeView::dataLogger() const
{
    return _loggerModel->dataLogger();
}

void BMS_DataLoggerTreeView::setDataLogger(DataLogger *dataLogger)
{
    _loggerModel->setDataLogger(dataLogger);
}

void BMS_DataLoggerTreeView::removeCurrent()
{
    if (_loggerModel->dataLogger() == nullptr)
    {
        return;
...
void BMS_DataLoggerTreeView::setColorCurrent()
{
...
void BMS_DataLoggerTreeView::exportOneCurrent() const
{
    DLData *dlData = currentData();
    if (dlData == nullptr)
    {
...
void BMS_DataLoggerTreeView::updateSelect(const QItemSelection &selected, const QItemSelection &deselected)
{
    Q_UNUSED(selected)
    Q_UNUSED(deselected)
...
void BMS_DataLoggerTreeView::createActions()
{
...
#endif
    connect(_removeAction, &QAction::triggered, this, &BMS_DataLoggerTreeView::removeCurrent);
    ...
    connect(_setColorAction, &QAction::triggered, this, &BMS_DataLoggerTreeView::setColorCurrent);
...
#endif
    connect(_exportOneAction, &QAction::triggered, this, &BMS_DataLoggerTreeView::exportOneCurrent);
    addAction(_exportOneAction);
}

QAction *BMS_DataLoggerTreeView::removeAction() const
{
    return _removeAction;
}

void BMS_DataLoggerTreeView::contextMenuEvent(QContextMenuEvent *event)
{
    QMenu menu;
